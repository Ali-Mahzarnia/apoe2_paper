...
mike_path='/Users/ali/Desktop/Jul/rna/apoe_mice/apoe2_mikes_sig_age/mikes_age.csv'
...
Ens_to_sym=read.delim('/Users/ali/Desktop/Jul/apoe2_paper/multi_cca/apoe2_mikes_sig_age/mikes_age.csv',sep="\t", header = T)
...
for (ii in 1:dim(RNA_result_ordered)[1]) {
  mikes_index=which( RNA_result_ordered$V1[ii]==mikes$Gene.Symbol )
  
  if (length(mikes_index)==0){
    Ens_to_sym_inedx=which(  RNA_result_ordered$V1[ii] == Ens_to_sym[dim(Ens_to_sym)[1],]     )
    Ens=Ens_to_sym[1,Ens_to_sym_inedx]
    mikes_index=which(Ens==mikes$Gene )
  }
  if (length(mikes_index)==0){log2FC_result[ii]=NA}
  else{log2FC_result[ii]=mikes$log2.FC.[mikes_index]}
