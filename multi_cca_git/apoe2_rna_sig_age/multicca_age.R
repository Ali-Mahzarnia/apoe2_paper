...
riskfactors=riskfactors%>%select(Sex, Age_Months)
...
########filter data by rna's significance
rna_path='###/rna_age.csv'
rnas=read.csv(rna_path) #already sorted by fdr pvalue
rna_names=rnas$Gene.Symbol
rna_names_significant=rna_names[rnas$fdr_p<=0.05]
rna_names_significant=subset(rna_names_significant, nchar(as.character(rna_names_significant))> 3)
indece_rna=na.omit(as.numeric(match(rna_names_significant, names(RNA_data))))

RNA_data=RNA_data[,indece_rna]
#check
names(RNA_data) %in%rna_names_significant
dim(RNA_data)
...
desired_sparsity_z=0 #.9995 and 0.9905343
...
##### add log2....number from rnas excel sheet
rnas_path='/Users/ali/Desktop/Jun23/apoe2_paper/divide_by_sum/multi_cca/apoe2_rnas_sig_sex/rnas.csv'
rnas=read.csv(rnas_path)


Ens_to_sym=read.delim('/Users/ali/Desktop/Jun23/apoe2_paper/divide_by_sum/multi_cca/apoe2_rnas_sig_sex/micewithsymbol.txt',sep="\t", header = T)
Ens_to_sym=t(Ens_to_sym)



log2FC_result=matrix(NA,dim(RNA_result_ordered)[1],1)
for (ii in 1:dim(RNA_result_ordered)[1]) {
  rnas_index=which( RNA_result_ordered$V1[ii]==rnas$Gene.Symbol )
  
  if (length(rnas_index)==0){
    Ens_to_sym_inedx=which(  RNA_result_ordered$V1[ii] == Ens_to_sym[dim(Ens_to_sym)[1],]     )
    Ens=Ens_to_sym[1,Ens_to_sym_inedx]
    rnas_index=which(Ens==rnas$Gene )
  }
  if (length(rnas_index)==0){log2FC_result[ii]=NA}
  else{log2FC_result[ii]=rnas$log2.FC.[rnas_index]}
...
agecat= case_when(  riskfactors$Age_Months<median(riskfactors$Age_Months) ~1 ,
                    riskfactors$Age_Months>=median(riskfactors$Age_Months) ~2)

#Dietname=Diet; Diet[Diet==1]="Control"; Diet[Diet==2]="HFD";
#Sexname=Sex; Sexname[Sexname=="1M"]="Male" ;  Sexname[Sexname=="2F"]="Female";

agecat[agecat==1]="1 Young"
agecat[agecat==2]="2 Old"
