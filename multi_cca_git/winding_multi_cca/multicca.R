...
riskfactors1=riskfactors%>%select(Sex, Age_Months)
#riskfactors2=riskfactors%>%select(NormSWDist, Distance, Winding)
riskfactors2=riskfactors%>%select( NormSWDist, Winding)

inddz1=0
for (i in 1:dim(riskfactors1)[2]) if(sd(riskfactors1[,i])==0 ) {inddz1=rbind(inddz1,i);  cat ( i , sd(riskfactors1[,i]), "\n" );}
if (length(inddz1)>1){
  inddz1=inddz1[2:dim(inddz1)[1]]
  riskfactors1=riskfactors1[,-inddz1]
}
...
library(PMA)
xlist= list (riskfactors1, image, riskfactors2)
...
connectivityexample=connectivity[,,1]#[-noreadcsf,-noreadcsf,1]
...
Sex=riskfactors$Sex;
Sexnames=Sex ; Sexnames[Sex=="male"]="1 Male"; Sexnames[Sex=="female"]="2 Female"; 
...
agecat=response$age_cat
###### here specify   
xaxisvar=Sexnames; 
xaxis="Sexnames" ;
xaxisvarnames=Sexnames;
brightnessvar=agecat; 
brightness="agecat";
#######
...
jpeg("violin_winding_sex.jpeg", units="in", width=20, height=10, res=300)  
...
agecat[agecat==0]="1 Young"
agecat[agecat==1]="2 Old"
